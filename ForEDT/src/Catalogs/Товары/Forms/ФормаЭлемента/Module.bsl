...
Процедура ДополнительнаяПроверка(Параметры)
...
Процедура КартинкаПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	...
	Иначе
		...
		ОпПослеПомещенияФайлов = Новый ОписаниеОповещения("ПослеПомещенияФайлов", ЭтотОбъект, ДопПараметры);
		НачатьПомещениеФайла(ОпПослеПомещенияФайлов,, ПеретаскиваемаяКартинка, Ложь, УникальныйИдентификатор);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПослеПомещенияФайлов(Результат, Адрес, ВыбранноеИмяФайла, ДопПараметры) Экспорт
	
	Файл = Новый Файл(ВыбранноеИмяФайла);
	ПослеПомещенияФайловНаСервере(Адрес, Файл.Имя, ДопПараметры);
	
КонецПроцедуры

&НаСервере
Процедура ПослеПомещенияФайловНаСервере(Адрес, ВыбранноеИмяФайла, ДопПараметры)
